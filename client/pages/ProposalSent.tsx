  ...
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 -z-10">
      {pieces.map((_, i) => {
        ...
        const bg = ["#FF6B6B", "#FFD93D", "#6BCB77", "#4D96FF", "#9B6BFF"][
    ...
    <section className="mx-auto w-full max-w-3xl px-6 py-20 text-center">
      ...
      <div className="mx-auto max-w-xl rounded-2xl border border-border/60 bg-white/90 p-10">
        ...
        <p className="mt-4 text-sm text-foreground/70">
          Your proposal has been sent to the selected vendors. Please wait for
          confirmation from the vendors — we expect responses within 24 hours.
        </p>
        <div className="mt-6 grid gap-3">
          <div className="rounded-md bg-primary/10 p-4 text-left text-sm">
            <strong>Proposal ID:</strong> {proposal.id}
            <div className="mt-2">
              We sent your request to the selected providers. You’ll receive
              confirmation or updates as soon as possible (max 24h).
            </div>
          </div>
          <Link
            to="/account"
          ...
          <button
            onClick={() => navigate("/")}
            className="rounded-full bg-primary/90 px-4 py-2 text-white"
          >
            Back to home
          </button>
        </div>
